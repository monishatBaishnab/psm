    ...
    return (
        ...
        >
            <button className="">{item.label}</button>
            {
                ...
                <div
                    className={`absolute flex left-0 top-full bg-psm-success-600 transition-all ${
                        !show ? "opacity-0 invisible" : "opacity-100 visible h-auto"
                    }`}
                >
                    {item?.children?.map((subItem) => (
                        <div className="cursor-auto text-left w-56">
                            <h5 className="px-4 py-3 block text-lg font-semibold">
                                {subItem.label}
                            </h5>
                            {subItem?.items?.map((text) => {
                                return (
                                    <button onClick={() => setShow(false)} className="text-left border-b last:border-b-0 border-b-psm-success-500/70 px-4 py-2 w-full hover:bg-psm-success-700/40">
                                        {text}
                                    </button>
                                );
                            })}
